...
function stub(seed: Record<string, string> = {}): StorageLike & { seen: Record<string, string> } {
...
describe('extractTags', () => {
  it('pulls #tags out of a body, folded and deduplicated', () => {
    expect(extractTags('a #Work note about #work and #deep-focus')).toEqual(['work', 'deep-focus']);
  });
...
describe('persistence', () => {
  ...
  it('treats a well-formed object with no notes array as no notes yet', () => {
    expect(loadState(stub({ 'platypad.notes.v1': '{"activeId":"n1"}' }))).toEqual(emptyState());
  });
});
